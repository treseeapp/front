    ...
    <q-header class="bg-primary text-white">
      <q-toolbar>
        ...
        <span class="text-h4 font-prime-regular q-pl-sm vertical-middle">
          ...
          <q-badge class="" color="white" text-color="white" outline v-if="userRol === 'profesor'">
            Vista Profesor
          </q-badge>

          <q-badge class="" color="white" text-color="white" outline v-if="userRol === 'estudiante'">
            Vista Estudiante
          </q-badge>

        </q-toolbar-title>
        <q-btn :label="userRol.toLowerCase()==='estudiante'?'Cambiar a Profesor':'Cambiar a Estudiante'"
               flat icon="fas fa-user-graduate" @click="changeUserRol"/>
      ...
      <div class="row q-mt-md">
        ...
        <div class="col-8 ">
          ...
          <div class="row flex-center justify-evenly q-mb-sm">
            <q-btn outline icon="person" color="indigo" to="/DEMO/account">
            ...
            >
              <template v-slot="{ item, index }">
                <q-item-section class="q-pr-lg q-pl-lg q-pt-sm q-pb-sm">
                  ...
                  <q-chat-message
                    v-if="(item.usuario === logedUser.nombre) && item.tipo.toLowerCase()!=='join'"
                    :name="item.usuario"
                    :text="[item.contenido]"
                    sent
                    text-color="white"
    ...
    <q-dialog v-model="confirmDisconnect">
      <q-card>
        ...
        <q-card-actions align="right">
          ...
          <q-btn flat label="Desconectar" color="primary" v-close-popup to="/"/>
        </q-card-actions>
      </q-card>
    </q-dialog>
    <q-ajax-bar
...
<script>
  ...
  export default {
          ...
          * */
          {
            title: 'Dashboard',
            rolNeeded: 'profesor',
            link: '/DEMO/profesor/dashboard',
            icon: 'fas fa-chart-pie'
          },
          {
            title: 'Estudiantes',
            rolNeeded: 'profesor',
            link: '/DEMO/profesor/estudiantes',
            icon: 'fas fa-user-graduate'
          },
          {
            title: 'Empresas',
            rolNeeded: 'profesor',
            link: '/DEMO/profesor/empresas',
            icon: 'fas fa-city'
          },
          /*
          * Estudiante links
          * */
          {
            ...
            link: '/DEMO/estudiante/dashboard',
            icon: 'fas fa-chart-pie'
          },
          {
            title: 'Fichajes',
            rolNeeded: 'estudiante',
            link: '/DEMO/estudiante/fichajes',
    ...
    methods: {
      changeUserRol() {
        if (this.userRol.toLowerCase() == 'profesor') {
          this.userRol = 'estudiante';
          this.$router.push('/DEMO/estudiante/dashboard/')
        } else {
          this.userRol = 'profesor';
          this.$router.push('/DEMO/profesor/dashboard')
